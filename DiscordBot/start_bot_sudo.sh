#!/bin/bash
# Скрипт запуска Discord бота для Vintage Story с правами суперпользователя (Linux)
...
if [ "$EUID" -ne 0 ]; then
    echo -e "${YELLOW}Запуск с правами суперпользователя...${NC}"
    sudo "$0" "$@"
    exit $?
fi
...
# Получить реального пользователя, который запустил sudo
...
if [ -f "requirements.txt" ]; then
    if [ ! -d "venv" ]; then
        ...
        if [[ $install_deps =~ ^[Yy]$ ]]; then
            ...
            # Создаем venv с правами обычного пользователя
...
echo -e "${GREEN}[ИНФО] Запуск Discord бота с правами суперпользователя...${NC}"
